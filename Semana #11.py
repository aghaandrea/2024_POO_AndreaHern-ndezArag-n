...
class Inventario:
    ...
    def buscar_producto_por_nombre(self, nombre):
        encontrados = [p for p in self.productos.values() if nombre.lower() in p.obtener_nombre().lower()]
        return encontrados
    ...
    def guardar_inventario(self, archivo):
        with open(archivo, 'w') as f:
            json.dump({p_id: p.__dict__ for p_id, p in self.productos.items()}, f, indent=4)
